...
public class AutoRunTheLauncher extends CommandBase {
  ...
  public void execute() { 
    ...
    while (Timer.getFPGATimestamp() < (startTime + Constants.LAUNCH_RUN_TIME)) {
      ...
      if (Timer.getFPGATimestamp() > (startTime + Constants.LAUNCH_RUN_TIME - Constants.INDEX_TIME+3)) {
        indexer_subsystem.shooting();
      }
      
      if (Timer.getFPGATimestamp() > (startTime + Constants.LAUNCH_RUN_TIME - Constants.INDEX_TIME+1)) {
